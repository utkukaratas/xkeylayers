...
public:
    ...
    sigset_t sigset;
    bool debug;
    vector<TKeyMapping*> map;
    TKey* generated;
    timeval timeout;
    bool timeout_valid;
    
    void parse_mapping (Display* ctrl_conn, char* mapping, bool debug);
};
...
{
    TApplication* self = new TApplication;
    int dummy, ch;
    ...
    self->parse_mapping (self->ctrl_conn, mapping, self->debug);
...
void handle_key (TApplication *self, TKeyMapping *key,
...
TKeyMapping *parse_token (Display *dpy, char *token, Bool debug)
{
    TKeyMapping *km = NULL;
    KeySym    ks;
    char      *from, *to, *key;
...
void TApplication::parse_mapping (Display* ctrl_conn, char* mapping, bool debug)
{
    char     *token;
    TKeyMapping* nkm;
